...
from src.ui.base_layout import style_base_dashbord, style_base_layout
...
from src.components.dialog_attendace_results import attendance_result_dialog
...
def teacher_dashbord():
    ...
    with c2:
        ...
        if st.button("Logout", type="secondary", key="loginbackbtn"):
            st.session_state['is logged_in'] = False
...
def teacher_tab_take_attendance():
    ...
    subject_opetions = {f"{s['name']} - {s['subject_code']}": s['subject_id'] for s in subjects}

    col1, col2 = st.columns([3,1], vertical_alignment='bottom')

    with col1:
        selected_subject_label = st.selectbox('Select Subjects', options=list(subject_opetions.keys()))

    with col2:
        if st.button('Add Photos', type='primary', icon=':material/add_photo_alternate:', width='stretch'):
            add_photos_dialog()

    selected_subject_id = subject_opetions[selected_subject_label]

    st.divider()

    if st.session_state.attendance_images:
        st.header('Added Photos')
        gallery_clos = st.columns(4)

        for idx, img in enumerate(st.session_state.attendance_images):
            with gallery_clos[idx % 4]:
...
def teacher_tab_attendance_records():
    ...
    for r in records:
        ...
        data.append({
            ...
            "Time": datetime.fromisocalendar(ts).strftime("%Y-%M-%d %I:%M %p") if ts else "N'A",
    ...
    display_df = (summary.sort_values(by='ts_group', ascending=False)
                  [['Time', 'Subject', 'Subject Code', 'Attendance Stat']]   
                  )
...
def teacher_screen_login():
    ...
    st.space()
    st.space()
    teacher_username = st.text_input("Enter username", placeholder="praveena")
...
def techer_screen_register():
    ...
    teacher_name = st.text_input("Enter name", placeholder="praveena pawar")
